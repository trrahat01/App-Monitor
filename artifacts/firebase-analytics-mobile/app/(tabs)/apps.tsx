...
const starterApps: AppItem[] = [
  ...
  { id: 'travel', name: 'Travel App', category: 'Travel', packageName: 'com.travel.app', enabled: true, status: 'Needs attention', color: '#F6B85C' },
];
...
export default function AppsScreen() {
  ...
  useEffect(() => { AsyncStorage.getItem('analytics-apps').then((value) => { if (value) setItems(JSON.parse(value) as AppItem[]); }); }, []);
  ...
  const addApp = () => {
    if (!name.trim() || !packageName.trim()) { Alert.alert('Add an app', 'Enter both an app name and package name.'); return; }
    persist([...items, { id: Date.now().toString(), name: name.trim(), category: 'New app', packageName: packageName.trim(), enabled: true, status: 'Not connected', color: '#FF755C' }]);
    setName(''); setPackageName(''); setShowAdd(false);
  };
  return (
    <ScrollView style={styles.screen} contentContainerStyle={[styles.content, { paddingTop: Math.max(insets.top, 18) + 8, paddingBottom: Platform.OS === 'web' ? 34 : 28 }]} showsVerticalScrollIndicator={false}>
      <View style={styles.header}><View><Text style={styles.eyebrow}>PORTFOLIO</Text><Text style={styles.title}>Connected apps</Text></View><Pressable style={styles.addButton} onPress={() => setShowAdd(true)} testID="add-app"><Feather name="plus" size={18} color={colors.light.primaryForeground} /></Pressable></View>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }, eyebrow: { color: colors.light.mutedForeground, fontSize: 11, fontWeight: '700', letterSpacing: 1.2 }, title: { color: colors.light.foreground, fontSize: 25, fontWeight: '700', marginTop: 5 }, addButton: { width: 40, height: 40, borderRadius: 13, backgroundColor: colors.light.primary, alignItems: 'center', justifyContent: 'center' },
